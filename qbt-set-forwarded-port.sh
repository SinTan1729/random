...
wget --save-cookies=/tmp/cookies.txt --keep-session-cookies --header="Referer: $qbt_addr" --header="Content-Type: application/x-www-form-urlencoded" \
  --post-data="username=$qbt_username&password=$qbt_password" --output-document - --quiet "$qbt_addr/api/v2/auth/login"

# listen_port=$(wget --load-cookies=/tmp/cookies.txt --output-document - --quiet "$qbt_addr/api/v2/app/preferences" | jq '.listen_port')
#
# if [ ! "$listen_port" ]; then
